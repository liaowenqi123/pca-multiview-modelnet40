...
CHECKPOINT_DIR = ROOT_DIR / "checkpoints" / "v3"


def main():
    parser = argparse.ArgumentParser(description="v3: Gated multi-view + dim reduction")
    ...
    train_dataset = ModelNet40MultiView(
        train_points, train_labels, num_views=6, augment=True,
        cache_pca=not args.no_cache_pca,
    )
    test_dataset = ModelNet40MultiView(
        test_points, test_labels, num_views=6, augment=False,
        cache_pca=not args.no_cache_pca,
    )
    ...
    print(f"[模型] 融合路径输入维度: {model.num_fused * args.dim_reduce}")
    print(f"[模型] 置信度门控 bias 初始值: {model.confidence_proj.bias.item():.2f}")
    ...
    # ── 训练 ──
    print(f"\n{'='*60}")
    print(f"v3 ResNet18+6View Gated+DimReduce({args.dim_reduce}) | "
          f"{args.epochs} epochs | bs={args.batch_size} | lr={args.lr}")
    print(f"{'='*60}\n")
