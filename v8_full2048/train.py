...
与 V7 唯一区别：n_sample=2048，不再随机采样，
PointNet 自注意力处理全部 2048 个点。
"""
...
from common import (
    NUM_CLASSES, GRID_SIZE, ModelNet40MultiView,
    MultiViewResNetV7, train_one_epoch, evaluate,
)

ROOT_DIR = Path(__file__).resolve().parent.parent
CHECKPOINT_DIR = ROOT_DIR / "checkpoints" / "v8"


def main():
    parser = argparse.ArgumentParser(description="V8: full 2048 points")
    ...
    print(f"\n{'='*60}")
    print(f"V8 Full2048 | {args.epochs} eps | bs={args.batch_size}")
    print(f"{'='*60}\n")

    for epoch in range(start_epoch, args.epochs):
        t0 = time.time()
        tl, ta = train_one_epoch(model, train_loader, optimizer, criterion, device, epoch+1)
        vl, va = evaluate(model, test_loader, criterion, device)
        scheduler.step()
        t = time.time() - t0
        print(f"Epoch {epoch+1:3d}/{args.epochs} | "
              f"Train Loss: {tl:.4f} | Train Acc: {ta:.4f} | "
              f"Val Loss: {vl:.4f} | Val Acc: {va:.4f} | Time: {t:.1f}s")
        if va > best_acc:
            best_acc = va
