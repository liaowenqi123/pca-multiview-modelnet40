...
CHECKPOINT_DIR = ROOT_DIR / "checkpoints" / "v4"


def main():
    parser = argparse.ArgumentParser(description="V4: TinyCNN + Dodecahedron 6-View")
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--batch_size", type=int, default=64)
    parser.add_argument("--lr", type=float, default=0.001)
    parser.add_argument("--dropout", type=float, default=0.5)
    parser.add_argument("--num_workers", type=int, default=0)
    parser.add_argument("--base_ch", type=int, default=32, help="TinyCNN 基础通道数")
    parser.add_argument("--dim_reduce", type=int, default=32, help="融合前各视角降维目标")
    parser.add_argument("--grid_size", type=int, default=56, help="投影图分辨率")
    ...
    train_dataset = ModelNet40MultiView(
        ...
        num_views=6, projection="dodeca", grid_size=args.grid_size,
        augment=True, cache_pca=not args.no_cache_pca,
    )
    test_dataset = ModelNet40MultiView(
        test_points, test_labels,
        num_views=6, projection="dodeca", grid_size=args.grid_size,
        augment=False, cache_pca=not args.no_cache_pca,
    )
    ...
    # ── 模型 ──
    model = MultiViewResNetV4(
        in_channels=5, base_ch=args.base_ch,
        dim_reduce=args.dim_reduce, dropout=args.dropout,
    ).to(device)

    total = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    C_out = model.backbone.out_channels
    print(f"[模型] 参数: {total:,} (可训练 {trainable:,})")
    print(f"[模型] TinyCNN 输出: {C_out}d, 融合输入: {model.num_views * args.dim_reduce}d")
    print(f"[模型] 网格: {args.grid_size}×{args.grid_size}, 基础通道: {args.base_ch}")
    ...
    # ── 训练 ──
    print(f"\n{'='*60}")
    print(f"V4 TinyCNN+Dodeca6View | {args.epochs} eps | bs={args.batch_size} | lr={args.lr}")
    print(f"{'='*60}\n")
