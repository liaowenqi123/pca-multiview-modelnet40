"""
V5: 冻结 V3 的 ResNet18 backbone，仅训练投影层 + 分类头。

    与 V3 架构完全相同，区别：backbone 参数 requires_grad = False。
    可训练参数 ~1M（V3 的 1/12），预期过拟合大幅缓解 + 训练加速。
"""
...
CHECKPOINT_DIR = ROOT_DIR / "checkpoints" / "v5"


def main():
    parser = argparse.ArgumentParser(description="V5: Frozen ResNet18 + Gated")
    ...
    model = MultiViewResNetV3(
        ...
        dim_reduce=args.dim_reduce, pretrained=not args.no_pretrain,
    ...
    print(f"[模型] 融合输入维度: {model.num_fused * args.dim_reduce}")
    ...
    # ── 训练 ──
    print(f"\n{'='*60}")
    print(f"V5 Frozen ResNet18+Gated | {args.epochs} eps | bs={args.batch_size}")
    print(f"{'='*60}\n")
